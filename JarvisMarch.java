...
public class JarvisMarch extends Algorithm{
     ...
     * 
     * Finds the vertex with the smallest polar angle to the current one.
     * Only look at vertices greater than the current y coordinate (less than because flipped).
     * Draw lines to vertices we're looking at in a red line.
     * If a smaller polar angle is found, save that one to return later.
     * 
     ...
     * 
     * Finds the vertex with the largest (most negative) negative polar angle.
     * Only look at vertices less than the current y coordinate (greater than because flipped).
     * Draw lines to vertices we're looking at in a red line.
     * If a smaller polar angle is found, save that one to return later.
     * 
     * @param current the vertex currently on
    ...
    /**
     * Run the algorithm
     * Copy vertices into vector to work with.
     * label all the vertices
     * Set the pen radius for drawing lines.
     * Get the top vertex, which is where we will stop
     * Go up to the top and then back down finding the next, adding it to the graph,
     * adding it to the hull, and pausing for animation.
     * When the current vertex is back to our starting point, quit.
     */
    @Override
